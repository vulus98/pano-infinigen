...
python -m infinigen.datagen.manage_jobs --output_folder outputs/outdoor/$folder_name --num_scenes $num_scenes \
    ...
    --pipeline_overrides LocalScheduleHandler.use_gpu=True manage_datagen_jobs.num_concurrent=$num_concurrent \
    --overrides camera.camera_pose_proposal.pitch=90 camera.camera_pose_proposal.roll=0 \
    --wandb_mode disabled

# Generate indoor
# python -m infinigen.datagen.manage_jobs --output_folder outputs/indoor/$folder_name --num_scenes $num_scenes 
